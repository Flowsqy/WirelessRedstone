...
public class R_Item {
    ...
    public void initialize(DeviceType itemType) {
        if (V_Manager.getItemStack() == null || V_Manager.getItemStack().length != 2) {
            WirelessRedstone.Log(new U_Log(U_Log.LogType.ERROR,
                    "Invalid materials for " + itemType.toString() + ". Disabling WirelessRedstone."));
    ...
    private void set() {
        ...
        if (value != null) {
            final var material = Registry.MATERIAL.get(NamespacedKey.minecraft(value));
        ...
        if (itemStack == null) {
            ...
            final var key = type.getKeyOrNull();
            final var keyName = key == null ? type.toString() : key.getKey();

            if (key != null) {

                if (itemType.equals(DeviceType.RedstoneSender)) {
                    if (C_Utility.hasData("SenderBlockType"))
                        C_Utility.getData("SenderBlockType").setValue(keyName);
                } else {
                    if (C_Utility.hasData("ReceiverBlockType"))
                        C_Utility.getData("ReceiverBlockType").setValue(keyName);
                }
                C_Utility.save();
            }
            if (!value.equals("null")) {
                if (itemType.equals(DeviceType.RedstoneSender))
                    WirelessRedstone.Log(new U_Log(U_Log.LogType.WARNING, "Invalid configuration for SenderBlockType. '"
                            + value + "' is not a solid block. Block type has been set to '" + keyName + "'."));
                else
                    WirelessRedstone
                            .Log(new U_Log(U_Log.LogType.WARNING, "Invalid configuration for ReceiverBlockType. '"
                                    + value + "' is not a solid block. Block type has been set to '" + keyName + "'."));
            }
        }
    ...
    private void createItemStack() {
        ...
        if (itemType.equals(DeviceType.RedstoneSender)) {
            ...
            lore.add(ChatColor.RESET + "" + ChatColor.GRAY
                    + ChatColor.translateAlternateColorCodes('&', C_Value.getSenderItemLore())
                    + ChatColor.translateAlternateColorCodes('&', secretCode));
        } else {
            secretCode = " &7&l &8&o &0&k &r ";
            displayname = ChatColor.WHITE + "" + U_Device.getDeviceNameWithColor(itemType);
            lore.add(ChatColor.RESET + "" + ChatColor.GRAY
                    + ChatColor.translateAlternateColorCodes('&', C_Value.getReceiverItemLore())
                    + ChatColor.translateAlternateColorCodes('&', secretCode));
        }
    ...
    public void createRecipe() {
        NamespacedKey key = new NamespacedKey(WirelessRedstone.getPlugin(),
                itemType.toString().toLowerCase().replace("redstone", "redstone_"));
